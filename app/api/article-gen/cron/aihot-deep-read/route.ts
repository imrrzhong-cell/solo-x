...
import { generateAihotDeepRead } from "@/lib/article-gen/generator";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const article = await generateAihotDeepRead(today);
    return NextResponse.json({ ok: true, date: today, title: article.title, chars: article.content_md.length });
  } catch (err: any) {
    console.error("AIHOT deep read generation error:", err);

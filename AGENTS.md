...
- **`app/api/`** — Route handlers for auth and Codex CLI proxying.
...
The admin at `/admin` is a local-only tool that spawns `Codex` CLI via `lib/admin/Codex.ts` to execute management commands. Authentication is cookie-based via `middleware.ts` with password from `ADMIN_PASSWORD` env var.
